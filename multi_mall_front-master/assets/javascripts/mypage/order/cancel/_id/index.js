...
const prefix = 'mypage/order/cancel/_id';
...
export default {
  ...
  methods: {
    ...
    init() {
      ...
      // init cancel price
      const paidProductPrice = this.orderProduct.product_discount_price ? this.orderProduct.product_discount_price : this.orderProduct.product_price;
      this.cancelProductPrice = (paidProductPrice + this.orderProduct.option_price) * this.orderProduct.amount;

      if (this.orderProduct.fee_pay_method !== 'CSH_DLVRY') {
    ...
    async requestCancel() {
      ...
      if (!that.order_product_cancel.reason_sub_type) isEmptyList.push('취소 사유');
      if (!that.order_product_cancel.reason) isEmptyList.push('취소 사유 내용');
