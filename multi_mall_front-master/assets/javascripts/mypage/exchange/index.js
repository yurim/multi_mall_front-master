      ...
      for (let orderTableIdx = 0; orderTableIdx < orderTableLength; orderTableIdx += 1) {
        ...
        for (let storesIdx = 0; storesIdx < storesLength; storesIdx += 1) {
          ...
          for (let deliveryGroupsIdx = 0; deliveryGroupsIdx < deliveryGroupsLength; deliveryGroupsIdx += 1) {
            if (!deliveryGroups[deliveryGroupsIdx].is_all_claimed) {
              deliveryGroups[deliveryGroupsIdx].delivery_fee = 0;
            }
            if (deliveryGroups[deliveryGroupsIdx].fee_pay_method !== 'CSH_DLVRY') {
              paidPrice += deliveryGroups[deliveryGroupsIdx].delivery_fee;
            } else { // 배송비가 착불인 경우
              const orderStatus = deliveryGroups[deliveryGroupsIdx].order_products[0].order_status;
              if (orderStatus !== 'CNCL') { // 취소 제외 (교환,반품) 인 경우 착불배송비 이미 지불되었으므로 총 결제금액에 포함
                paidPrice += deliveryGroups[deliveryGroupsIdx].delivery_fee;
              }
            }

            paidPrice += deliveryGroups[deliveryGroupsIdx].product_total_price;
        ...
        this.orderTable[orderTableIdx].claim_paid_price = paidPrice;
      }
    },
    // init
    init() {
