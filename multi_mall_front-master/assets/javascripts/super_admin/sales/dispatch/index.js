...
export default {
  data: () => ({
    loading: false,
    headers: [
      { text: '주문번호', align: 'start', value: 'order_num', sortable: false, fixed: true, class: 'fixed', width: '130px' },
      { text: '상품주문번호', value: 'product_order_num', sortable: false, fixed: true, class: 'fixed', width: '140px' },
      { text: '판매매장', value: 'store_name', sortable: false, width: '140px', align: 'center' },
          ...
          async okCallback(params) {
            if (params.file) {
              ...
              if (res.data.result === 'success') {
                await this.$store.dispatch(`${prefix}/GetDataTable`, that.$route.query);
              } else {
