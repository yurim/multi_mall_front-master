...
export default {
  ...
  methods: {
    ...
    setTotalPrice() {
      ...
      this.maxUsablePoint = Math.ceil((totalProductPrice + totalDeliveryPrice - totalSalePrice) * 0.1);
    ...
    makeTempOrderData() {
      ...
      tempOrderData.channel = 'PC'; // pc or mobile
    ...
    async requestPay(tempOrder) {
      ...
      this.$IMP().request_pay(payData, async (resultSuccess) => {
        await this.paymentSuccessCallback(tempOrder, resultSuccess);
      }, (resultFailure) => {
        this.paymentFailureCallback(resultFailure);
      });
    },
    async paymentSuccessCallback(tempOrder, resultSuccess) {
      // 성공할 때 실행 될 콜백 함수
      await this.$createLoading(async () => {
        const res = await this.$store.dispatch(`${prefix}/complete_payment`, resultSuccess);
        if (res.data.result === 'error') return this.$popup.showAlertPopup(res.data.message);
        if (res.data.data.token) Utils.addCookie('jwtNonMember', res.data.data.token);
        ...
        this.$router.replace(`/order/completed/${resultSuccess.merchant_uid}`);
      });
    },
    paymentFailureCallback(resultFailure) {
      // 실패시 실행 될 콜백 함수
      let msg = '결제에 실패하였습니다.';
      msg += `에러내용 : ${resultFailure.error_msg}`;
      this.$popup.showAlertPopup(msg);
    },
    selectDlvryMsg() {
