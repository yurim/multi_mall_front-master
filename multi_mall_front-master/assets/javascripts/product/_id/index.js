...
export default {
  mixins: [RecentProductsMixin, UserAuthMixin, SelperCommonMixin],
  data() {
    return {
  ...
  async fetch({ store, params, redirect }) {
    // product
    const res = await store.dispatch(`${prefix}/getProduct`, params.id);
    if (res.result === 'error') return redirect('/main');
  ...
  methods: {
    ...
    async addCarts() {
      ...
      if (this.isLogin && this.userGrade === 0) { // 로그인
        ...
        this.$store.dispatch(`${prefixCart}/get_carts`);
        ...
        this.$store.dispatch(`${prefixCart}/get_unknown_carts`, { unknown_carts: unknownCarts });
        this.popupAddedCarts();
      }
    ...
    async naverPayBuy() {
      ...
      if (this.$selperMode.indexOf(this.$nodeEnv) > -1) {
        this.npayPurchaseSelper(tempOrder);
      }
      if (window.facebookPixelId && window.fbq) {
        let numItems = 0;
        let value = 0;
    ...
    npayPurchaseSelper(tempOrder) {
      const intv = setInterval(() => {
        ...
        if (__trackSelperPurchase) {
          ...
          __selperProductOrder.__selperOSType = 'pc';
