...
const prefix = 'store_admin/sales/return_delivery';
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { sortable: false, text: '반품(환불)완료일', value: '' },
  ...
  methods: {
    ...
    btnCollect() {
      ...
      if (!['반품요청'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('반품요청 상태의 주문 상품만 수거 처리할 수 있습니다.');
    ...
    btnCollectComplete() {
      ...
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (!['상품 수거중', '반품요청'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('반품요청 또는 상품 수거중 상태의 주문 상품만 수거 완료 처리할 수 있습니다.');
    ...
    btnReturnComplete() {
      // 반품 완료 처리 - 상품수거중 또는 수거완료 상태에서만 가능
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (['환불보류'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('환불 보류를 해제한 뒤 진행할 수 있습니다.');
      if (!['상품 수거중', '수거완료'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('수거중 또는 수거 완료 상태의 주문 상품만 반품 완료 처리할 수 있습니다.');
    ...
    btnReturnReject() {
      ...
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (!['반품요청', '수거완료'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('반품요청, 수거완료 상태의 주문 상품만 반품거부 처리할 수 있습니다.');
    ...
    btnChange() {
      ...
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (['환불완료'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('반품 완료되지 않은 주문 상품만 교환으로 변경할 수 있습니다.');
    ...
    btnRefundPostpone() {
      ...
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (!['수거완료'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('수거완료 상태의 주문 상품만 환불 보류 설정할 수 있습니다.');
    ...
    btnRefundPostponeCancel() {
      ...
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (!['환불보류'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('환불보류 상태의 주문 상품만 환불보류 해제할 수 있습니다.');

      new this.$popup.Confirm({
        propsData: {
          title: '선택하신 주문건 1건에 대한 환불 보류 설정을 해제하시겠습니까?',
          okCallback: async (params) => {
    ...
    btnEditReturnReason() {
      ...
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (['환불완료'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('반품 완료되지 않은 주문 상품만 사유를 수정할 수 있습니다.');
    ...
    btnEditFeePayMethod() {
      ...
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (!['반품요청'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('반품 요청 상태의 주문 상품만 반품 배송비 지불 방법을 수정할 수 있습니다.');
