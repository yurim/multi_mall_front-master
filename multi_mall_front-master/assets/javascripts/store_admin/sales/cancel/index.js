...
const prefix = 'store_admin/sales/cancel';
...
export default {
  data: () => ({
    ...
    headers: [
      ...
      { text: '취소(환불)완료일', value: '', sortable: false },
  ...
  methods: {
    ...
    btnCancelReject() {
      ...
      if (!['취소요청'].includes(this.selectedItem.order_sub_status)) return this.$popup.showAlertPopup('취소 요청 상태인 주문 건만 취소 거부 처리할 수 있습니다.');
    ...
    btnCancelAccept() {
      if (!this.selectedItem) return this.$popup.showAlertPopup('주문 상품을 선택해주세요.');
      if (!['취소요청'].includes(this.selectedItem.order_sub_status)) {
