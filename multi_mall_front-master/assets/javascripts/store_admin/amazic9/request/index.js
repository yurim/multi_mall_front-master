...
const prefix = 'store_admin/amazic9/request';
...
export default {
  ...
  async fetch({ store, params, redirect }) {
    if (!params.orderId || !params.orderProductIds) return redirect('/store_admin/amazic9');
    const query = {
      order_id: params.orderId,
      order_product_ids: params.orderProductIds,
    };
    await store.dispatch(`${prefix}/getOrderInfo`, query);
  ...
  methods: {
        ...
        deliveryProducts.push({
          ...
          info: {
            ...
            product: {
              ...
              additional_images: (orderProduct.product) ? orderProduct.product.additional_images : null,
    ...
    initCommonCodes() {
      ...
      this.delivery_types = deliveryTypes.filter((v) => v.key === 'SHIPPING');
    },
    /**
     * 다음 주소검색
     */
    ...
    makeDeliveryProductsData() {
      ...
      this.deliveryProducts.forEach((item) => {
        ...
        if (item.option_image.type === 'select') {
    ...
    checkRequestData(deliveryRequest, deliveryProducts) {
      ...
      for (let i = 0; i < deliveryProducts.length; i += 1) {
        ...
        if (item.target !== 'PDD' && !item.tracking_num) return '해외트래킹넘버를 입력해주세요';
