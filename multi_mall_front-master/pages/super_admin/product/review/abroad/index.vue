                  ...
                  <div class="table_size_s">
                    ...
                    >
                      <template v-slot:body="{item}">
                        ...
                        <td>
                          <span class="text">{{ item.review_type_str }}</span>
                        </td>
                        <td><span>{{ item.writer_id }}</span></td>
                        <td><span class="text"><a v-on:click="detailReview(item)">{{ item.translation_content }}</a></span></td>
                        <td>
                          <span class="thumbnail_wrap" v-if="item.review_type === 'PHOTO'">
                            <DefaultImage :imageUrl="item.image_url" />
                          </span>
                        </td>
                        <td><span class="text">{{ item.score }}</span></td>
                        <td><span class="text">{{ item.store_name }}</span></td>
                        <td><span class="text">{{ item.category }}</span></td>
                        <td><span class="text">{{ $moment(item.created_at).format('YYYY-MM-DD') }}</span></td>
                        <td>
                          <span class="text" v-if="item.is_show">노출</span>
...
<script src="@/assets/javascripts/super_admin/product/review/abroad/index.js"></script>
