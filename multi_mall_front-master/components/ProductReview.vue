<template>
  <!-- 상품리뷰 내용-->
  <div>
    <div class="color_gray_70" v-show="!comparePrice">
      상품을 구매하신 분들이 작성하신 리뷰 입니다. 리뷰 작성시 아래 금액 만큼 포인트가 적립 됩니다.
      <br>
      텍스트리뷰 : {{ textReviewPoint | comma }}원 I 포토 리뷰 : {{ photoReviewPoint | comma }}원
    </div>
    <div class="review_show_wrap">
      <!-- *** START 리뷰 평점 *** -->
      <div class="product_title m_t_30">
        <span>리뷰 평점</span>
      </div>
      ...
      <div class="review_summary_area">
        <div class="review_area">
          ...
          <div class="num_bar">
            <div class="bar_wrap">
              <v-progress-linear :value="progressLinearValueScoreStats(product.score_stats, 'score_5', product.total_review_count + product.abroad_total_review_count)" class="vue_progress"></v-progress-linear>
            </div>
            <span class="review_total">{{ reviewScoreStr(product.score_stats, 'score_5') | comma }}</span>
          </div>
        </div>
        <div class="review_area">
          <div class="data_star">
          ...
          <div class="num_bar">
            <div class="bar_wrap">
              <v-progress-linear :value="progressLinearValueScoreStats(product.score_stats, 'score_4', product.total_review_count + product.abroad_total_review_count)" class="vue_progress"></v-progress-linear>
            </div>
            <span class="review_total">{{ reviewScoreStr(product.score_stats, 'score_4') | comma }}</span>
          </div>
        </div>
        <div class="review_area">
          <div class="data_star">
          ...
          <div class="num_bar">
            <div class="bar_wrap">
              <v-progress-linear :value="progressLinearValueScoreStats(product.score_stats, 'score_3', product.total_review_count + product.abroad_total_review_count)" class="vue_progress"></v-progress-linear>
            </div>
            <span class="review_total">{{ reviewScoreStr(product.score_stats, 'score_3') | comma }}</span>
          </div>
        </div>
        <div class="review_area">
          <div class="data_star">
          ...
          <div class="num_bar">
            <div class="bar_wrap">
              <v-progress-linear :value="progressLinearValueScoreStats(product.score_stats, 'score_2', product.total_review_count + product.abroad_total_review_count)" class="vue_progress"></v-progress-linear>
            </div>
            <span class="review_total">{{ reviewScoreStr(product.score_stats, 'score_2') | comma }}</span>
          </div>
        </div>
        <div class="review_area">
          <div class="data_star">
          ...
          <div class="num_bar">
            <div class="bar_wrap">
              <v-progress-linear :value="progressLinearValueScoreStats(product.score_stats, 'score_1', product.total_review_count + product.abroad_total_review_count)" class="vue_progress"></v-progress-linear>
            </div>
            <span class="review_total">{{ reviewScoreStr(product.score_stats, 'score_1') | comma }}</span>
          </div>
        </div>
      </div>
      <!-- *** END 리뷰 평점 *** -->
      ...
      <div class="product_title">
        <span>
          베스트 포토리뷰
          <em>{{ product.best_review_count | comma }}</em>건</span>
<!--        <ul class="tab_menu_wrap">-->
<!--          <li><a href="javascript:;" class="on">더보기</a></li>-->
<!--        </ul>-->
      </div>
      <div v-if="0 < bestReviews.length">
      ...
      <div class="tab_menu_wrap">
        <div class="product_title">
          <span>해외 구매리뷰 <em>{{ product.abroad_total_review_count | comma }}</em>건</span>
        </div>
...
<!--        </ul>-->
      ...
      <div class="gray_tab_menu_wrap">
        ...
        <div class="checkbox_wrap pull-right" v-show="!comparePrice">
      ...
      <div class="tab_menu_wrap">
        <div class="product_title">
            <span>국내 구매리뷰 <em>{{ product.total_review_count | comma }}</em>건</span>
        </div>
              ...
              <li v-if="moreReview || !moreReview && i < 2" v-for="(review, i) in reviews" :key="review.id">
                <div class="review_show_wrap__info_wrap">
                  ...
                  <div class="review_show_wrap__info_wrap_option"><span>{{ review.name }}</span> ㅣ
                    <span>{{ review.created_at | date }}</span> ㅣ <span v-if="review.option_values">옵션 : [{{ review.option_values }}] | </span> <span> {{ review.amount | comma }}개 </span></div>
