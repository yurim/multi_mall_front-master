...
export default {
    ...
    window.$(`#${this.summernoteId}`).summernote({
      ...
      callbacks: {
        ...
        onImageUpload: async (files) => {
          ...
          await util.asyncFor(filesLength, async (idx) => {
            ...
            await this.summernoteInsertImage(imageToBlob);
            return true;
          });
          this.$forceUpdate();
        },
            ...
            imageTags.forEach((tag, index) => {
              ...
              if (dImage) {
                detailImages.push({
                  image: dImage.image,
                  order: index + 1,
                  original_order: dImage.original_order,
                  is_image_file: false,
                });
  ...
  methods: {
    summernoteInsertImage(blob) {
      return new Promise((resolve) => {
        window.$(`#${this.summernoteId}`).summernote('insertImage', blob, () => {
          resolve();
        });
      });
    },
