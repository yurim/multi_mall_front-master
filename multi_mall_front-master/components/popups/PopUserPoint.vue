            ...
            <div class="popup_text_wrap">
              <ul class="list_wrap">
                ...
                <li>
                  ...
                  <div class="body">
                    <select v-if="params.type === 'save'" v-model="params.reason">
                      <option value="">선택</option>
                      <option :value="item.value" v-for="(item, i) in savingReason" :key="`save_${i}`">{{ item.text }}</option>
                    </select>
                    <select v-if="params.type === 'use'" v-model="params.reason">
                      <option value="">선택</option>
                      <option :value="item.value" v-for="(item, i) in recoveryReason" :key="`use_${i}`">{{ item.text }}</option>
                    </select>
                  </div>
                </li>
                <li>
                  ...
                  <div class="body">
                    <input type="number" placeholder="포인트를 입력하세요." v-model="params.point" class="w_100" />
                  </div>
                </li>
              </ul>
            </div>
