              ...
              <div class="table_wrap">
                <table>
                  ...
                  <tr>
                    ...
                    <td>
                      <div class="even_child_wrap">
                        <select v-for="(categories, i) in copiedCategories" v-bind:key="i"
                    ...
                    <td>
                      <div class="d_ib_100 m_b_10">
                        ...
                        <template v-if="exchanges">
                          ...
                          <div class="input_div_wrap" v-for="exchange in exchanges">
                            <div class="l_h_34">{{ `${exchange.name} (${exchange.unit}) = ` }}</div>
                            <input type="number" placeholder="현재 환율 입력" v-model="exchange.rate">
                          </div>
                        </template>
                        <!-- *** 화폐단위 변환 END *** -->
                        ...
                        <div>
                          <div class="radio_wrap m_b_10">
                            <input type="radio" id="is_change_price_false" :value="false" v-model="isChangePrice">
                            <label for="is_change_price_false">수정안함</label>
                            <input type="radio" id="is_change_price_true" :value="true" v-model="isChangePrice">
                        ...
                        <div v-if="isChangePrice">
                          ...
                          <div class="radio_wrap m_b_10">
                            <input type="radio" id="extra_price_type_rate" value="rate" v-model="extraPriceType">
                            <label for="extra_price_type_rate">배율</label>
                            <input type="radio" id="extra_price_type_number" value="number" v-model="extraPriceType">
                            <label for="extra_price_type_number">특정금액</label>
                            <input type="radio" id="extra_price_type_compound" value="compound" v-model="extraPriceType">
                            <label for="extra_price_type_compound">복합</label>
                          </div>

                          <div class="input_but_wrap">
                            <input v-show="extraPriceType === 'compound' || extraPriceType === 'rate'"
                                   class="m_b_10" type="number" placeholder="% 입력" v-model="extraPriceRate">
                            <input v-show="extraPriceType === 'compound' || extraPriceType === 'number'"
                                   class="m_b_10" type="number" placeholder="특정금액 입력" v-model="extraPriceNumber">
                          </div>
                          ...
                          <div>
                            ...
                            <div class="radio_wrap m_b_10">
                              <input type="radio" id="additional_price_type_rate" value="product_price" v-model="additionalPriceType">
                              <label for="additional_price_type_rate">판매가에만 적용</label>
                              <input type="radio" id="additional_price_type_all" value="all" v-model="additionalPriceType">
                  ...
                  <tr v-if="isAbroad">
                    <th>해외상품정보고시 (상단)</th>
                    <td>
                      <select v-model="selectedABRDProductTopNoticeTemplate">
                        <option value="">선택</option>
                        <option v-for="item in templates.ABRD_PRDCT_TOP_NTC" :key="item.id" :value="item">{{ item.name }}</option>
                      </select>
                    </td>
                  </tr>
                  <tr v-if="isAbroad">
                    <th>해외상품정보고시 (하단)</th>
                    <td>
                      <select v-model="selectedABRDProductBtmNoticeTemplate">
                        <option value="">선택</option>
                        <option v-for="item in templates.ABRD_PRDCT_BTM_NTC" :key="item.id" :value="item">{{ item.name }}</option>
                      </select>
                    </td>
                  </tr>
                  <!-- *** 해외상품정보고시 템플릿 END *** -->
